...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, src_mask=None):
        x = x + self.self_attn(self.ln1(x), self.ln1(x), mask=src_mask, is_causal=False)
...
class Encoder(nn.Module):
    def __init__(self, vocab_size, d_model, num_layers, num_heads, d_ff, dropout=0.1, rope=False, rel_bias=False):
        ...
        self.layers = nn.ModuleList([EncoderLayer(d_model, num_heads, d_ff, dropout, rope, rel_bias) for _ in range(num_layers)])
